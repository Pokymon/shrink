...
impl Authenticable for Model {
  async fn find_by_api_key(
    ...
    let user = users::Entity
      ::find()
      .filter(
        model::query::condition().eq(users::Column::ApiKey, api_key).build()
      )
      .one(db).await?;
...
impl Model {
  ...
  pub async fn find_by_magic_token(
    ...
    let user = users::Entity
      ::find()
      .filter(
        query::condition().eq(users::Column::MagicLinkToken, token).build()
      )
      .one(db).await?;
  ...
  pub async fn find_by_reset_token(
    ...
    let user = users::Entity
      ::find()
      .filter(
        model::query::condition().eq(users::Column::ResetToken, token).build()
      )
      .one(db).await?;
    user.ok_or_else(|| ModelError::EntityNotFound)
  }
  ...
  pub async fn find_by_pid(
    ...
    let parse_uuid = Uuid::parse_str(pid).map_err(|e|
      ModelError::Any(e.into())
    )?;
    let user = users::Entity
      ::find()
      .filter(
        model::query::condition().eq(users::Column::Pid, parse_uuid).build()
      )
      .one(db).await?;
    user.ok_or_else(|| ModelError::EntityNotFound)
  }
...
impl ActiveModel {
  ...
  pub async fn set_email_verification_sent(
    ...
    self.email_verification_sent_at = ActiveValue::set(
      Some(Local::now().into())
    );
    self.email_verification_token = ActiveValue::Set(
  ...
  pub async fn create_magic_link(
    ...
    let expired =
      Local::now() + Duration::minutes(MAGIC_LINK_EXPIRATION_MIN.into());
